...
import { applyCumulativeFilters, applySearchFilter, applySorting } from '../../../shared/utils/mobileFilters'
import { normalizePositions } from '../../../shared/utils/currencyNormalization'
                        ...
                        {groupByBaseSymbol && expandedNetSymbols.has(pos.symbol) && pos.variants && pos.variants.length > 0 && (
                          pos.variants.map((variant, vIdx) => (
                            <div key={`${idx}-v-${vIdx}`} className="flex text-[10px] text-[#6B7280] bg-[#EEF2FF] border-b border-[#C7D2FE] border-l-4 border-l-indigo-400">
                              ...
                              {netVisibleColumns.netVolume && <div className="flex items-center justify-start px-1 h-[40px] min-w-[80px] flex-shrink-0 bg-[#EEF2FF] text-[#6B7280]">{formatNum(variant.exactSymbol?.endsWith('.c') ? variant.netVolume / 100 : variant.netVolume)}</div>}
